...
import { collection, query, onSnapshot, doc, setDoc, where, getDocs, updateDoc } from 'firebase/firestore';
...
import { SeasonManagementCard } from './components/SeasonManagementCard';
...
export default function OwnerDashboard() {
  ...
  const handleCreatePitch = async (e: React.FormEvent) => {
    ...
    if (!newPitch.name || !newPitch.adminEmail) {
      toast.error(isArabic ? 'اسم الملعب والبريد الإلكتروني للمسؤول مطلوبان' : 'Name and Admin Email are required');
      return;
    }
    ...
    try {
      ...
      const pitchData: Pitch = {
        ...
        pricePerHour: Number(newPitch.pricePerHour) || 350,
      ...
      toast.success(isArabic ? 'تم إنشاء الملعب بنجاح!' : 'Pitch created successfully');
      setNewPitch({});
    } catch (err: unknown) {
  ...
  const handleUpdateAdminRole = async (email: string) => {
    try {
      ...
      if (qs.empty) {
        toast.info(
          isArabic
            ? `لم يتم العثور على مستخدم بهذا البريد: ${email}. اطلب منه تسجيل الدخول أولاً.`
            : `No user found with email ${email}. Instruct the user to sign in once so they can be assigned.`
        );
        return;
      }
      ...
      toast.success(isArabic ? `تم ترقية المستخدم إلى مسؤول لـ ${email}` : `User role updated to admin for ${email}`);
    } catch (err: unknown) {
      const error = err as Error;
