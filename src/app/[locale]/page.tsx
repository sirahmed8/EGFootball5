...
export default async function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="flex min-h-screen flex-col overflow-hidden">
      <section className="relative flex flex-col items-center justify-center min-h-[90vh] text-center px-4">
        ...
        <div className="relative z-10 max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
            ...
            <Link href="/login" className={cn(buttonVariants({ size: 'lg', variant: 'outline' }), "text-xl px-10 py-8 rounded-full font-bold border-border hover:bg-accent hover:text-accent-foreground hover:scale-[1.02] active:scale-95 transition-all w-full sm:w-auto")}>
              {t('signInBtn')}
            </Link>
          </div>
        </div>
      ...
      <section className="py-32 bg-background relative z-10 px-4 border-t border-border">
