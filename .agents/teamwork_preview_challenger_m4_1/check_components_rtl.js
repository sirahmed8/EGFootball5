...
function findTsxFiles(dir) {
  ...
  list.forEach(file => {
    ...
    // Ignore src/components/ui primitives
    if (fullPath.includes(path.join('src', 'components', 'ui'))) return;
...
const files = findTsxFiles(compDir);
...
console.log(`--- Custom Components RTL Utility Audit Report ---`);
console.log(`Scanned ${files.length} .tsx files in src/components (excluding ui/)`);
